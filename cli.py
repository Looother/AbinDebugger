...
def load_settings():
    ...
    if config_path.exists():
        ...
        DebugController.APP_SETTINGS = {
            ...
            "DB_HOST": "localhost",
            "DB_PORT": "27017",
            "DEBUG_DB_NAME": "Bugfixes",
            "DEBUG_DB_PATTERNS_COLLECTION": "BugPatterns",
...
def main():
    ...
    AbinLogging.debugging_logger.setLevel(logging.INFO)
    ...
    if args.mine:
        ...
        if bugfixes_data:
            print("Connecting to MongoDB to insert patterns...")
            client = MongoClient(
                f"mongodb://{DebugController.APP_SETTINGS.get('DB_HOST', 'localhost')}:{DebugController.APP_SETTINGS.get('DB_PORT', '27017')}"
            )
            db = client[DebugController.APP_SETTINGS.get("DEBUG_DB_NAME", "Bugfixes")]
            db["RepoData"].insert_one(repo_data.copy())
            db[DebugController.APP_SETTINGS.get("DEBUG_DB_PATTERNS_COLLECTION", "BugPatterns")].insert_many(bugfixes_data.copy())
            print(f"Successfully inserted {len(bugfixes_data)} bug patterns into MongoDB!")
        else:
